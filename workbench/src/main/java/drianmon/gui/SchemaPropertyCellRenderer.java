    ...
    extends javax.swing.table.DefaultTableCellRenderer
{
    ...
    public SchemaPropertyCellRenderer(Workbench wb) {
        ...
        listRenderer = new JComboBox(DrianmonGuiDef.Measure._aggregator_values);
        ...
        int column)
    {
        if (column == 1) {
            ...
            //targetClassz == DrianmonGuiDef.Formula.class &&
            if (propertyName.equals("cdata")) {
                 ...
                 */
                       ...
                       == DrianmonGuiDef.OrdinalExpression.class)
            {
                SchemaPropertyCellRenderer spcr =
                       ...
                       == DrianmonGuiDef.CalculatedMemberProperty.class)
            {
                ...
                return tableRenderer;
                       ...
                       == DrianmonGuiDef.RelationOrJoin.class)
            {
    ...
    private String generateJoinStr(Object value) {
        DrianmonGuiDef.Join currentJoin = (DrianmonGuiDef.Join) value;
        String joinStr = "<html>"
    ...
    private String generateLeftRightStr(Object value) {
        DrianmonGuiDef.RelationOrJoin currentObj =
            (DrianmonGuiDef.RelationOrJoin) value;
        if (currentObj instanceof DrianmonGuiDef.Table) {
            return (((DrianmonGuiDef.Table) currentObj).alias == null
                    || ((DrianmonGuiDef.Table) currentObj).alias.equals("")
                ? ((DrianmonGuiDef.Table) currentObj).name
                : ((DrianmonGuiDef.Table) currentObj).alias);
        }
        DrianmonGuiDef.Join currentJoin = (DrianmonGuiDef.Join) currentObj;
        String joinStr = "("
            + generateLeftRightStr(currentJoin.left)
            + " <b>JOIN</b> "
            + generateLeftRightStr(currentJoin.right)
    ...
    void setTableRendererHeight(JTable relationTable, JPanel relationRenderer) {
        ...
        for (int i = 0; i < relationTable.getRowCount(); i++) {
            ...
            try {
                ...
                if (value instanceof DrianmonGuiDef.RelationOrJoin) {
