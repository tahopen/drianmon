    ...
    extends javax.swing.tree.DefaultTreeCellEditor
{
    ...
    public void setValueAt(JTree tree) {
        String retValue;
        DrianmonGuiDef.RelationOrJoin relationObj = null;

        retValue = (String) getCellEditorValue();
        if (retValue.equals("Join")) {
            relationObj =
                new DrianmonGuiDef.Join(
                    "", "",
                    new DrianmonGuiDef.Table(
                        "", "Table 1", "", null),
                    "", "",
                    new DrianmonGuiDef.Table(
                        "", "Table 2", "", null));
        } else if (retValue.equals("Table")) {
            relationObj = new DrianmonGuiDef.Table("", "Table", "", null);
        }
        ...
        if (tpath != null) {
            ...
            if (parentpath != null) {
                ...
                if (parent instanceof DrianmonGuiDef.Hierarchy) {
                    ((DrianmonGuiDef.Hierarchy) parent).relation = relationObj;
                } else if (parent instanceof DrianmonGuiDef.Closure) {
                    ((DrianmonGuiDef.Closure) parent).table =
                        (DrianmonGuiDef.Table)relationObj;
                } else if (parent instanceof DrianmonGuiDef.Join) {
                    int indexOfChild =
                        tree.getModel().getIndexOfChild(parent, value);
                    switch (indexOfChild) {
                    case 0:
                        ((DrianmonGuiDef.Join) parent).left = relationObj;
                        break;
                    case 1:
                        ((DrianmonGuiDef.Join) parent).right = relationObj;
                        break;
                    }
                }
