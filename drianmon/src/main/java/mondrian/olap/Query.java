 ...
 *
 * <p>
 * It is created by calling {@link Connection#parseQuery}, and executed by calling {@link Connection#execute}, to return
 * a {@link Result}.
 * </p>
 *
 * <h3>Query control</h3>
 *
 * <p>
 * Most queries are model citizens, executing quickly (often using cached results from previous queries), but som
 * queries take more time, or more database resources, or more results, than is reasonable. Mondrian offers three ways
 * to control rogue queries:
 * <ul>
 *
 * <li>You can set a query timeout by setting the {@link MondrianProperties#QueryTimeout} parameter. If the query takes
 * longer to execute than the value of this parameter, the system will kill it.</li>
 *
 * <li>The {@link MondrianProperties#QueryLimit} parameter limits the number of cells returned by a query.</li>
 *
 * <li>At any time while a query is executing, another thread can cancel the query by calling {@link #getStatement()}.
 * {@link Statement#cancel() cancel()}. The call to {@link Connection#execute(Query)} will throw an exception.</li>
 *
 * </ul>
 *
...
public class Query extends QueryPart {
  ...
  /**
   * Unique list of members referenced from the measures dimension. Will be used to determine if cross joins can be
   * processed natively for virtual cubes.
   */
  private Set<Member> measuresMembers;

  /**
   * If true, virtual cubes can be processed using native cross joins. It defaults to true, unless functions are applied
   * on measures.
   */
  private boolean nativeCrossJoinVirtualCube;

  /**
   * Used for virtual cubes. Comtains a list of base cubes related to a virtual cube
   */
  ...
  /**
   * How should the query be returned? Valid values are: ResultStyle.ITERABLE ResultStyle.LIST ResultStyle.MUTABLE_LIST
   * For java4, use LIST
   */
  ...
  /**
   * List of aliased expressions defined in this query, and where they are defined. There might be more than one aliased
   * expression with the same name.
   */
  private final List<ScopedNamedSet> scopedNamedSets = new ArrayList<>();
  private boolean ownStatement;

  /**
   * Creates a Query.
   */
  public Query( Statement statement, Formula[] formulas, QueryAxis[] axes, String cube, QueryAxis slicerAxis,
      QueryPart[] cellProps, boolean strictValidation ) {
    this( statement, Util.lookupCube( statement.getSchemaReader(), cube, true ), formulas, axes, slicerAxis, cellProps,
   ...
   * Creates a Query.
   */
  public Query( Statement statement, Cube mdxCube, Formula[] formulas, QueryAxis[] axes, QueryAxis slicerAxis,
      QueryPart[] cellProps, Parameter[] parameters, boolean strictValidation ) {
  ...
  /**
   * Adds a new formula specifying a member or a set to an existing query; resolve is called after the formula has been
   * added.
   *
   * @param formula
  ...
  /**
   * Adds some number of new formulas specifying members or sets to an existing query; resolve is only called once,
   * after all the new members have been added to the query.
   *
   * @param additions
  ...
  public Validator createValidator() {
    return createValidator( statement.getSchema().getFunTable(), false, new HashMap<>() );
  }

  public Validator createValidator( Map<QueryPart, QueryPart> resolvedIdentifiers ) {
    return createValidator( statement.getSchema().getFunTable(), false, resolvedIdentifiers );
  }
   ...
   *          Whether to always resolve function definitions (see {@link Validator#alwaysResolveFunDef()})
   * @return Validator
   */
  public Validator createValidator( FunTable functionTable, boolean alwaysResolveFunDef ) {
    return new QueryValidator( functionTable, alwaysResolveFunDef, Query.this, new HashMap<>() );
  }
  ...
  /**
   * @deprecated Please use {@link #clone}; this method will be removed in mondrian-4.0
   */
  ...
  public Query clone() {
    return new Query( statement, cube, Formula.cloneArray( formulas ), QueryAxis.cloneArray( axes ),
        ( slicerAxis == null ) ? null : (QueryAxis) slicerAxis.clone(), cellProps, parameters.toArray(
            new Parameter[parameters.size()] ), strictValidation );
  }
  ...
  /**
   * Issues a cancel request on this Query object. Once the thread running the query detects the cancel request, the
   * query execution will throw an exception. See <code>BasicQueryTest.testCancel</code> for an example of usage of this
   * method.
   *
  ...
  /**
   * Checks if either a cancel request has been issued on the query or the execution time has exceeded the timeout value
   * (if one has been set). Exceptions are raised if either of these two conditions are met. This method should be
   * called periodically during query execution to ensure timely detection of these events, particularly before/after
   ...
   * @return start time
   *
   * @deprecated Use {@link Execution#getStartTime}. This method is deprecated and will be removed in mondrian-4.0
   */
   ...
   *
   * <p>
   * This method is called automatically when a query is created; you need to call this method manually if you have
   * modified the query's expression tree in any way.
   */
  public void resolve() {
  ...
  /**
   * @return true if the relevant property for ignoring invalid members is set to true for this query's environment (a
   *         different property is checked depending on whether environment is schema load vs query validation)
   */
  public boolean ignoreInvalidMembers() {
    MondrianProperties props = MondrianProperties.instance();
    final boolean load = ( (RolapCube) getCube() ).isLoadInProgress();
    return !strictValidation && ( load ? props.IgnoreInvalidMembers.get() : props.IgnoreInvalidMembersDuringQuery
        .get() );
  }

  /**
   * A Query's ResultStyle can only be one of the following: ResultStyle.ITERABLE ResultStyle.LIST
  ...
  /**
   * Returns a collection of all axes, including the slicer as the first element, if there is a slicer.
   *
   ...
   *
   * <p>
   * If the expression's type is a dimension with several hierarchies, assumes that the expression yields a member of
   * the first (default) hierarchy of the dimension.
   *
   * <p>
   ...
   *       {[Measures].[Unit Sales], [Measures].[Store Cost]})</code> </blockquote>
   *
   * has type <code>{[Time.Monthly], [Measures]}</code> even though <code>[Time].LastSibling</code> might return a
   * member of either [Time.Monthly] or [Time.Weekly].
   */
  ...
  public void setParameter( final String parameterName, final Object value ) {
    ...
    final Object value2 =
        Locus.execute( new Execution( statement, 0 ), "Query.quickParse", new Locus.Action<Object>() {
   ...
   * <li>For strings, takes string, or calls {@link Object#toString()} on any other type
   * <li>For members, takes member or string
   * <li>For sets of members, requires a list of members or strings and converts each element to a member.
   * </ul>
   *
   * @param type
   *          Type
    ...
    switch ( category ) {
      ...
      case Category.Set:
        ...
        if ( !( value instanceof List ) ) {
          throw Util.newInternal( "Invalid value '" + value + "' for parameter '" + parameterName + "', type "
              + type );
        }
   ...
   *          If true, schema reader returns only elements which are accessible to the statement's current role
   *
  ...
  /**
   * Looks up a member whose unique name is <code>memberUniqueName</code> from cache. If the member is not in cache,
   * returns null.
   */
  public Member lookupMemberFromCache( String memberUniqueName ) {
    ...
    for ( Member member : getDefinedMembers() ) {
      if ( Util.equalName( member.getUniqueName(), memberUniqueName ) || Util.equalName( getUniqueNameWithoutAll(
  ...
  private String getUniqueNameWithoutAll( Member member ) {
    ...
    if ( ( parentMember != null ) && !parentMember.isAll() ) {
      return Util.makeFqName( getUniqueNameWithoutAll( parentMember ), member.getName() );
    } else {
  ...
  /**
   * Remove a formula from the query. If <code>failIfUsedInQuery</code> is true, checks and throws an error if formula
   * is used somewhere in the query.
   */
  public void removeFormula( String uniqueName, boolean failIfUsedInQuery ) {
    ...
    if ( failIfUsedInQuery && formula != null ) {
      ...
      while ( walker.hasMoreElements() ) {
        ...
        // or formula
        String formulaType =
            formula.isMember() ? MondrianResource.instance().CalculatedMember.str() : MondrianResource
        ...
        while ( ( parent != null ) && ( grandParent != null ) ) {
          if ( grandParent instanceof Query ) {
            if ( parent instanceof Axis ) {
              throw MondrianResource.instance().MdxCalculatedFormulaUsedOnAxis.ex( formulaType, uniqueName,
                  ( (QueryAxis) parent ).getAxisName() );

            } else if ( parent instanceof Formula ) {
              String parentFormulaType =
                  ( (Formula) parent ).isMember() ? MondrianResource.instance().CalculatedMember.str()
                      : MondrianResource.instance().CalculatedSet.str();
              throw MondrianResource.instance().MdxCalculatedFormulaUsedInFormula.ex( formulaType, uniqueName,
                  parentFormulaType, ( (Formula) parent ).getUniqueName() );
        ...
        throw MondrianResource.instance().MdxCalculatedFormulaUsedInQuery.ex( formulaType, uniqueName, Util.unparse(
            this ) );
      }
    }
  ...
  /**
   * Returns whether a formula can safely be removed from the query. It can be removed if the member or set it defines
   * it not used anywhere else in the query, including in another formula.
   *
   * @param uniqueName
  ...
  public boolean canRemoveFormula( String uniqueName ) {
    ...
    while ( walker.hasMoreElements() ) {
      ...
      if ( queryElement instanceof NamedSetExpr && ( (NamedSetExpr) queryElement ).getNamedSet().equals(
          mdxElement ) ) {
        return false;
      }
    }
  ...
  List<Member> getDefinedMembers() {
    ...
    for ( final Formula formula : formulas ) {
      if ( formula.isMember() && formula.getElement() != null && getConnection().getRole().canAccess( formula
          .getElement() ) ) {
  ...
  /**
   * Returns <code>Hierarchy[]</code> used on <code>axis</code>. It calls {@link #collectHierarchies}.
   */
  ...
  public ExpCompiler createCompiler() {
    ...
    return createCompiler( evaluator, validator, Collections.singletonList( resultStyle ) );
  }

  private ExpCompiler createCompiler( final Evaluator evaluator, final Validator validator,
      List<ResultStyle> resultStyleList ) {
  ...
  /**
   * @return set of members from the measures dimension referenced within this query
   */
  ...
  /**
   * Put an Object value into the evaluation cache with given key. This is used by Calc's to store information between
   * iterations (rather than re-generate each time).
   *
   * @param key
   *          the cache key
   * @param value
   ...
   *
   * <p>
   * Note especially that {@link #getCalculatedMember(java.util.List)} returns the calculated members defined in this
   * query. It does not perform access control; all calculated members defined in a query are visible to everyone.
   */
  private static class QuerySchemaReader extends DelegatingSchemaReader implements NameResolver.Namespace {
    ...
    @Override
    public Member getMemberByUniqueName( List<Id.Segment> uniqueNameParts, boolean failIfNotFound,
        MatchType matchType ) {
      ...
      if ( member == null ) {
        ...
        member = schemaReader.getMemberByUniqueName( uniqueNameParts, failIfNotFound, matchType );
      }
    ...
    @Override
    public OlapElement lookupCompoundInternal( OlapElement parent, List<Id.Segment> names, boolean failIfNotFound,
        int category, MatchType matchType ) {
      if ( matchType == MatchType.EXACT ) {
        OlapElement oe = lookupCompound( parent, names, failIfNotFound, category, MatchType.EXACT_SCHEMA );
        if ( oe != null ) {
          return oe;
        }
      }
    ...
    public List<NameResolver.Namespace> getNamespaces() {
      final List<NameResolver.Namespace> list = new ArrayList<>();
      list.add( this );
  ...
  /**
   * Implementation of {@link drianmon.olap.Validator} that works within a particular query.
   *
   * <p>
   * It's unlikely that we would want a validator that is NOT within a particular query, but by organizing the code this
   * way, with the majority of the code in {@link drianmon.olap.ValidatorImpl}, the dependencies between Validator and
   * Query are explicit.
   */
  private static class QueryValidator extends ValidatorImpl {
     ...
     *          Function table
     * @param alwaysResolveFunDef
     *          Whether to always resolve function definitions (see {@link #alwaysResolveFunDef()})
     * @param query
     *          Query
     */
  ...
  /**
   * Schema reader that depends on the current scope during the validation of a query. Depending on the scope, different
   * calculated sets may be visible. The scope is represented by the expression stack inside the validator.
   */
  private static class ScopedSchemaReader extends DelegatingSchemaReader implements NameResolver.Namespace {
    ...
    @Override
    public OlapElement lookupCompoundInternal( OlapElement parent, final List<Id.Segment> names,
        boolean failIfNotFound, int category, MatchType matchType ) {
      switch ( category ) {
        case Category.Set:
        case Category.Unknown:
          final ScopedNamedSet namedSet =
              queryValidator.getQuery().lookupScopedNamedSet( names, queryValidator.getScopeStack() );
          if ( namedSet != null ) {
            return namedSet;
          }
      }
    ...
    public OlapElement lookupChild( OlapElement parent, IdentifierSegment segment ) {
      ...
      return queryValidator.getQuery().lookupScopedNamedSet( Collections.singletonList( Util.convert( segment ) ),
  ...
  public static class ScopedNamedSet implements NamedSet {
     ...
     *          Name
     * @param scope
     *          Scope of named set (the function call that encloses the 'expr AS name', often GENERATE or FILTER)
     * @param expr
  ...
  private class ParameterFinder extends MdxVisitorImpl {
    ...
    public Object visit( UnresolvedFunCall call ) {
      if ( call.getFunName().equals( "Parameter" ) ) {
        ...
        Parameter parameter = new ParameterImpl( parameterName, Literal.nullValue, null, type );
        parameters.add( parameter );
  ...
  /**
   * Visitor that locates and registers all aliased expressions ('expr AS alias') as named sets. The resulting named
   * sets have scope, therefore they can only be seen and used within that scope.
   */
      ...
      if ( exp instanceof FunCall ) {
        ...
        if ( call2.getSyntax() == Syntax.Infix && call2.getFunName().equals( "AS" ) ) {
          ...
          if ( call2.getArg( 1 ) instanceof Id ) {
            ...
            createScopedNamedSet( ( (Id.NameSegment) id.getSegments().get( 0 ) ).getName(), parent, call2.getArg(
                0 ) );
          } else if ( call2.getArg( 1 ) instanceof NamedSetExpr ) {
            NamedSetExpr set = (NamedSetExpr) call2.getArg( 1 );
            createScopedNamedSet( set.getNamedSet().getName(), parent, call2.getArg( 0 ) );
          }
        }
      }
    }
