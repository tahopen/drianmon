...
import drianmon.rolap.aggmatcher.AggGen;
            ...
            for (RolapStar.Measure measure : measuresList) {
                if (measure.getAggregator().isDistinct()
                    && measure.getExpression() instanceof
                        DrianmonDef.MeasureExpression)
                {
                    DrianmonDef.MeasureExpression measureExpr =
                        (DrianmonDef.MeasureExpression) measure.getExpression();
                    DrianmonDef.SQL measureSql = measureExpr.expressions[0];
