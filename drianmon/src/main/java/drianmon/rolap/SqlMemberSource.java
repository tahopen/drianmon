...
import drianmon.olap.Property;
import drianmon.olap.Util;
import drianmon.rolap.agg.AggregationManager;
import drianmon.rolap.agg.CellRequest;
import drianmon.rolap.aggmatcher.AggStar;
import drianmon.rolap.sql.MemberChildrenConstraint;
import drianmon.rolap.sql.MemberKeyConstraint;
...
import drianmon.util.CreationException;
import drianmon.util.ObjectFactory;
    ...
    implements MemberReader, SqlTupleReader.MemberBuilder
{
    ...
    public int countMemberChildren( Member member, List<RolapMember> children, MemberChildrenConstraint constraint ) {
        return countMemberChildren( (RolapMember) member, children, constraint);
    }
        ...
        boolean[] mustCount)
    {
        ...
        if (!sqlQuery.getDialect().allowsFromQuery()) {
            ...
            for (int i = levelDepth; i >= 0; i--) {
                ...
                DrianmonDef.Expression keyExp = level2.getKeyExp();
...
*/
    ...
    private int countMemberChildren(RolapMember parentMember, List<RolapMember> children, MemberChildrenConstraint constraint) {
        ...
        try {
            ...
            if(resultSet.next()) {
                ...
                RolapUtil.LOGGER.debug(
                  "[mondrian.sql] SqlMemberSource.countMemberChildren" + ": count returned  [" + result + "] records"
                );
                return result;
            }
            RolapUtil.LOGGER.error(
              "Exception: The query [" + sql + "] returned no values" );
        ...
        throw MondrianResource.instance().Internal.ex("Exception: The query [" + sql + "] returned no values");
    }
        ...
        DataSource dataSource)
    {
        ...
        for (RolapLevel level : levels) {
            ...
            final DrianmonDef.Expression keyExp = level.getKeyExp();
            hierarchy.addToFrom(sqlQuery, keyExp);
            ...
            if (!keyExp.equals(level.getOrdinalExp())) {
                ...
                final DrianmonDef.Expression ordinalExp =
                    level.getOrdinalExp();
            ...
            for (RolapProperty property : properties) {
                final DrianmonDef.Expression propExpr = property.getExp();
    ...
    {
        return makeChildMemberSqlQuery(member, dataSource, constraint).toSqlAndTypes();
    }
      ...
      boolean applyOrderBy)
    {
        ...
        if (level.hasCaptionColumn()) {
            DrianmonDef.Expression captionExp = level.getCaptionExp();
            if (!levelCollapsed) {
        ...
        RolapProperty[] properties = level.getProperties();
        for (RolapProperty property : properties) {
            final DrianmonDef.Expression exp = property.getExp();
            if (!levelCollapsed) {
    ...
    public static boolean levelContainsMultipleColumns(RolapLevel level) {
        ...
        DrianmonDef.Expression keyExp = level.getKeyExp();
        DrianmonDef.Expression ordinalExp = level.getOrdinalExp();
        DrianmonDef.Expression captionExp = level.getCaptionExp();
     ...
     * 'All' level).
     */
    private Pair<String, List<SqlStatement.Type>> makeChildMemberSqlPCRoot(
        RolapMember member)
    {
        return makeChildMemberSqlPCRootQuery(member).toSqlAndTypes();
    }
    private SqlQuery makeChildMemberSqlPCRootQuery(
      RolapMember member)
    {
        SqlQuery sqlQuery =
          SqlQuery.newQuery(
    ...
    private void addLevel(
        ...
        boolean group)
    {
        final DrianmonDef.Expression key = level.getKeyExp();
        final DrianmonDef.Expression order = level.getOrdinalExp();
    ...
    private static class RolapParentChildMember extends RolapMemberBase {
         ...
         * @see drianmon.olap.Member#getDepth()
         */
        public int getDepth() {
            return depth;
        }
    ...
    /**
     * Default {@link drianmon.rolap.SqlMemberSource.ValuePoolFactory}
     ...
     * {@link drianmon.rolap.SqlMemberSource.NullValuePoolFactory}. </p>
     */
