...
import drianmon.olap.fun.MondrianEvaluationException;
import drianmon.rolap.*;
import drianmon.util.Bug;
import drianmon.xmla.XmlaHandler;
...
public abstract class MondrianOlap4jConnection implements OlapConnection {
     ...
     */
    final Map<drianmon.olap.Schema, MondrianOlap4jSchema> schemaMap =
        new HashMap<drianmon.olap.Schema, MondrianOlap4jSchema>();
     ...
     * See {@link drianmon.olap4j.MondrianOlap4jDriver} for more details.
     *
        ...
        throws SQLException
    {
        ...
        this.mondrianConnection =
            (RolapConnection) drianmon.olap.DriverManager
    ...
    MondrianOlap4jCube toOlap4j(drianmon.olap.Cube cube) {
        MondrianOlap4jSchema schema = toOlap4j(cube.getSchema());
        return new MondrianOlap4jCube(cube, schema);
    }

    MondrianOlap4jDimension toOlap4j(drianmon.olap.Dimension dimension) {
        if (dimension == null) {
            return null;
        }
    ...
    Type toOlap4j(drianmon.olap.type.Type type) {
        if (type instanceof drianmon.olap.type.BooleanType) {
            return new BooleanType();
        } else if (type instanceof drianmon.olap.type.CubeType) {
            final drianmon.olap.Cube mondrianCube =
                ((drianmon.olap.type.CubeType) type).getCube();
            return new CubeType(toOlap4j(mondrianCube));
        } else if (type instanceof drianmon.olap.type.DecimalType) {
            drianmon.olap.type.DecimalType decimalType =
                (drianmon.olap.type.DecimalType) type;
            return new DecimalType(
                decimalType.getPrecision(),
                decimalType.getScale());
        } else if (type instanceof drianmon.olap.type.DimensionType) {
            drianmon.olap.type.DimensionType dimensionType =
                (drianmon.olap.type.DimensionType) type;
            return new DimensionType(
                toOlap4j(dimensionType.getDimension()));
        } else if (type instanceof drianmon.olap.type.HierarchyType) {
            return new BooleanType();
        } else if (type instanceof drianmon.olap.type.LevelType) {
            return new BooleanType();
        } else if (type instanceof drianmon.olap.type.MemberType) {
            final drianmon.olap.type.MemberType memberType =
                (drianmon.olap.type.MemberType) type;
        ...
        } else if (type instanceof drianmon.olap.type.NullType) {
            return new NullType();
        } else if (type instanceof drianmon.olap.type.NumericType) {
            return new NumericType();
        } else if (type instanceof drianmon.olap.type.SetType) {
            final drianmon.olap.type.SetType setType =
                (drianmon.olap.type.SetType) type;
            return new SetType(toOlap4j(setType.getElementType()));
        } else if (type instanceof drianmon.olap.type.StringType) {
            return new StringType();
        } else if (type instanceof drianmon.olap.type.TupleType) {
            drianmon.olap.type.TupleType tupleType =
                (drianmon.olap.type.TupleType) type;
            final Type[] types = toOlap4j(tupleType.elementTypes);
            return new TupleType(types);
        } else if (type instanceof drianmon.olap.type.SymbolType) {
    ...
    MondrianOlap4jMember toOlap4j(drianmon.olap.Member member) {
        if (member == null) {
            return null;
        }
    ...
    MondrianOlap4jLevel toOlap4j(drianmon.olap.Level level) {
        if (level == null) {
            return null;
        }
    ...
    MondrianOlap4jHierarchy toOlap4j(drianmon.olap.Hierarchy hierarchy) {
        if (hierarchy == null) {
            return null;
        }
    ...
    Type[] toOlap4j(drianmon.olap.type.Type[] mondrianTypes) {
    ...
    MondrianOlap4jNamedSet toOlap4j(
        drianmon.olap.Cube cube,
        drianmon.olap.NamedSet namedSet)
    {
        if (namedSet == null) {
            return null;
        }
    ...
    static class Helper {
            ...
            String msg, Throwable cause)
        {
            ...
            assert !drianmon.util.Bug.olap4jUpgrade(
    ...
    private static class MondrianToOlap4jNodeConverter {
        ...
        private org.olap4j.mdx.Syntax toOlap4j(drianmon.olap.Syntax syntax) {
